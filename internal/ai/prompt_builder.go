	...
	defaults := []PromptTemplate{
		{
			...
			Template:    "Title: {{.title}}\n\nArticle Type: {{.article_type}}\n\nTarget Length: {{.word_count}} words (minimum {{.word_count_min}} words; natural variation is fine, never below the minimum)\n\nKeywords: {{.keywords}}\n\nInstructions:\n{{.instructions}}\n\nResearch Briefing:\n{{.briefing}}\n\nOutline:\n{{.outline}}\n\nWriting requirements:\n- Write a complete, well-structured article with at least three H2 sections (## headings) and H3 subsections (###) where appropriate.\n- Develop each section substantially: support every claim with specific information (numbers, dates, names, examples) — never generic filler or placeholder statements.\n- Include practical examples. For list/comparison articles give every item real details (features, prices, pros, cons, limitations).\n- Incorporate the verified facts from the Research Briefing and reference the sources listed there. Never invent facts, statistics, quotes, dates, or sources.\n- End with a useful conclusion: the key takeaway or the reader's next step.\n- No padding, no repetition, no vague claims. Use clear sentences and scannable paragraphs.",
			Variables:   []string{"title", "article_type", "word_count", "word_count_min", "keywords", "instructions", "briefing", "outline", "tone", "audience"},
			Defaults:    map[string]string{"word_count": "1200", "word_count_min": "1000", "tone": "professional", "audience": "general"},
			Version:     "2.0",
		},
		{
			...
			Template:    "Título: {{.title}}\n\nTipo de Artigo: {{.article_type}}\n\nTamanho Alvo: {{.word_count}} palavras (mínimo de {{.word_count_min}} palavras; variação natural é aceitável, nunca abaixo do mínimo)\n\nPalavras-chave: {{.keywords}}\n\nInstruções:\n{{.instructions}}\n\nBriefing de Pesquisa:\n{{.briefing}}\n\nEsboço:\n{{.outline}}\n\nRequisitos de escrita:\n- Escreva um artigo completo e bem estruturado com pelo menos três seções H2 (cabeçalhos ##) e subseções H3 (###) quando apropriado.\n- Desenvolva cada seção substancialmente: sustente cada afirmação com informação específica (números, datas, nomes, exemplos) — nunca encha com lugar-comum.\n- Inclua exemplos práticos. Para artigos de lista/comparação dê a cada item detalhes reais (recursos, preços, prós, contras, limitações).\n- Incorpore os fatos verificados do Briefing de Pesquisa e cite as fontes listadas. Nunca invente fatos, estatísticas, citações, datas ou fontes.\n- Termine com uma conclusão útil: o principal aprendizado ou o próximo passo do leitor.\n- Sem enrolação, sem repetição, sem afirmações vagas. Use frases claras e parágrafos escaneáveis.",
			Variables:   []string{"title", "article_type", "word_count", "word_count_min", "keywords", "instructions", "briefing", "outline", "tone", "audience"},
			Defaults:    map[string]string{"word_count": "1200", "word_count_min": "1000", "tone": "profissional", "audience": "geral"},
			Version:     "2.0",
		},
		{
			...
			Template:    "Title: {{.title}}\n\nBriefing: {{.briefing}}\n\nKeywords: {{.keywords}}\n\nExpected Size: {{.word_count}} words\n\nCreate a detailed outline with at least three major sections (H2) and subsections (H3) where appropriate. Each section must be specific enough to be developed into substantial content — never a heading without an angle.",
			Variables:   []string{"title", "briefing", "keywords", "word_count"},
			Defaults:    map[string]string{"word_count": "1200"},
			Version:     "2.0",
		},
		{
			...
			Template:    "Título: {{.title}}\n\nBriefing: {{.briefing}}\n\nPalavras-chave: {{.keywords}}\n\nTamanho Esperado: {{.word_count}} palavras\n\nCrie um esboço detalhado com pelo menos três seções principais (H2) e subseções (H3) quando apropriado. Cada seção deve ser específica o suficiente para ser desenvolvida em conteúdo substancial — nunca um cabeçalho sem abordagem.",
			Variables:   []string{"title", "briefing", "keywords", "word_count"},
			Defaults:    map[string]string{"word_count": "1200"},
			Version:     "2.0",
		},
		...
		{
			ID:          PromptTypeDeepResearch,
			Name:        "Deep Research Synthesis",
			Description: "Synthesize multiple sources into a structured research summary",
			Language:    "en",
			System:      "You are a research analyst. Synthesize the provided sources into a structured research summary. Answer: what is new, which companies are involved, dates, numbers, versions, and context. Cite source URLs.",
			Template:    "Topic: {{.topic}}\n\nSources:\n{{.sources}}\n\nReturn JSON with keys: summary (string), key_points (string[]), data_found (string[]), statistics (string[]), dates (string[]), companies (string[]), products (string[]), conclusions (string[]).",
			Variables:   []string{"topic", "sources"},
			Version:     "1.0",
		},
		{
			ID:          PromptTypeDeepResearch + "_pt",
			Name:        "Síntese de Pesquisa Profunda",
			Description: "Sintetizar múltiplas fontes em um resumo estruturado de pesquisa",
			Language:    "pt",
			System:      "Você é um analista de pesquisa. Sintetize as fontes fornecidas em um resumo estruturado. Responda: o que há de novo, quais empresas estão envolvidas, datas, números, versões e contexto. Cite URLs das fontes.",
			Template:    "Tema: {{.topic}}\n\nFontes:\n{{.sources}}\n\nRetorne JSON com as chaves: summary (string), key_points (string[]), data_found (string[]), statistics (string[]), dates (string[]), companies (string[]), products (string[]), conclusions (string[]).",
			Variables:   []string{"topic", "sources"},
			Version:     "1.0",
		},
		{
			ID:          PromptTypeFactBase,
			Name:        "Fact Base Extraction",
			Description: "Extract structured facts from research sources",
			Language:    "en",
			System:      "You are a fact extractor. Extract verifiable structured facts from the sources. Only include facts directly supported by the source text.",
			Template:    "Topic: {{.topic}}\n\nSource Text:\n{{.content}}\n\nReturn JSON array with keys: type (company|product|version|price|date|event|technology|number), entity (string), value (string), source (URL string), confidence (0-100).",
			Variables:   []string{"topic", "content"},
			Version:     "1.0",
		},
		{
			ID:          PromptTypeFactBase + "_pt",
			Name:        "Extração de Base de Fatos",
			Description: "Extrair fatos estruturados das fontes de pesquisa",
			Language:    "pt",
			System:      "Você é um extrator de fatos. Extraia fatos estruturados verificáveis das fontes. Inclua apenas fatos diretamente suportados pelo texto da fonte.",
			Template:    "Tema: {{.topic}}\n\nTexto da Fonte:\n{{.content}}\n\nRetorne um array JSON com as chaves: type (company|product|version|price|date|event|technology|number), entity (string), value (string), source (URL), confidence (0-100).",
			Variables:   []string{"topic", "content"},
			Version:     "1.0",
		},
	}

...
func TestSelectInternalLinks_FiltersByScore(t *testing.T) {
	...
	siteID := uuid.New()
	posts := pgxmock.NewRows([]string{"id", "title", "slug"}).
	...
	// Publications (e.g. articles generated by the workflow engine) are
	// candidates too — Sprint 6.7: internal links must also come from
	// publications, not only from editor posts.
	pubs := pgxmock.NewRows([]string{"id", "title", "slug"}).
		AddRow(uuid.New(), "Novidades de inteligência artificial", "publicacao-ia").
		AddRow(uuid.New(), "Guia de jardinagem", "jardinagem-2026")
	m.ExpectQuery(`SELECT id, COALESCE\(title,''\), COALESCE\(slug,''\) FROM publications`).
		WithArgs(siteID, pgxmock.AnyArg()).
		WillReturnRows(pubs)
	...
	for _, l := range links {
		...
		if l.Slug == "publicacao-ia" {
			foundPublication = true
			if l.Relevance < 40 {
				t.Errorf("expected relevant publication candidate, got %f", l.Relevance)
			}
		}
		if l.Slug == "bolo-chocolate" || l.Slug == "jardinagem-2026" {
			t.Errorf("unrelated candidate %q must not pass the threshold", l.Slug)
		}
	}
	if !foundPublication {
		t.Error("expected a publication to be considered as internal link candidate")
	}
}

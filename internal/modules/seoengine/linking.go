...
// SelectInternalLinks queries the site's published posts AND its publications
// (every item exchanged through the publisher funnel — including articles
// generated by the workflow engine, which live in publications, not posts)
// and returns the best internal link candidates (composite score >= minScore,
// capped at maxLinks).
func (s *Service) SelectInternalLinks(ctx context.Context, siteID uuid.UUID, sourcePostID *uuid.UUID, sourceTitle, sourceContent, sourceKeyword, sourceCategory string, minScore, maxLinks int) ([]InternalLinkCandidate, error) {
	...
	collect := func(sql string) error {
		rows, err := p.Query(ctx, sql, siteID, exclude)
		if err != nil {
			return fmt.Errorf("failed to list linking candidates: %w", err)
		}
		defer rows.Close()
		for rows.Next() {
			...
			if err := rows.Scan(&postID, &title, &slug); err != nil {
				return fmt.Errorf("failed to scan linking candidate: %w", err)
			}
			...
			if _, seen := bySlug[key]; !seen {
				bySlug[key] = target{postID: postID, title: title, slug: slug}
			}
		}
		return rows.Err()
	}

	// Editor posts first, then everything the publisher ever published. A
	// generated article's own publication becomes a natural internal-link
	// target for later articles on the same site.
	if err := collect(
	...
	if err := collect(
		`SELECT id, COALESCE(title,''), COALESCE(slug,'') FROM publications
		 WHERE site_id = $1 AND status = 'published' AND slug <> '' AND id <> $2::uuid
		 ORDER BY published_at DESC NULLS LAST LIMIT 100`); err != nil {
		return nil, err
	}
	...
	for _, t := range bySlug {
		...
		scored := ScoreInternalLinkCandidate(sourceTitle, sourceContent, sourceKeyword, sourceCategory, t.title, t.slug, "")
		scored.PostID = t.postID

...
func TestHandler_Create(t *testing.T) {
	...
	t.Run("missing site", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPost, "/tags", strings.NewReader(`{"name":"Test"}`))
	...
	t.Run("invalid body", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPost, "/tags", strings.NewReader(`{invalid`))
		req.Header.Set("Content-Type", "application/json")
		req = withSiteID(req)
		rest.AdaptHandler(h.Create).ServeHTTP(rec, req)
		if rec.Code != http.StatusBadRequest {
...
func TestHandler_Update(t *testing.T) {
	...
	t.Run("missing site", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPut, "/tags/"+uuid.New().String(), strings.NewReader(`{"name":"Updated"}`))
		req.Header.Set("Content-Type", "application/json")
		req = withChiParams(req, map[string]string{"id": uuid.New().String()})
	...
	t.Run("invalid id", func(t *testing.T) {
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodPut, "/tags/invalid", strings.NewReader(`{"name":"Updated"}`))
		req.Header.Set("Content-Type", "application/json")
		req = withChiParams(req, map[string]string{"id": "invalid"})
		req = withSiteID(req)
		rest.AdaptHandler(h.Update).ServeHTTP(rec, req)
	...
	t.Run("invalid body", func(t *testing.T) {
		rec := httptest.NewRecorder()
		tagID := uuid.New().String()
		req := httptest.NewRequest(http.MethodPut, "/tags/"+tagID, strings.NewReader(`{invalid`))
		req.Header.Set("Content-Type", "application/json")
		req = withChiParams(req, map[string]string{"id": tagID})
		req = withSiteID(req)
		rest.AdaptHandler(h.Update).ServeHTTP(rec, req)
	...
	t.Run("db error", func(t *testing.T) {
		rec := httptest.NewRecorder()
		tagID := uuid.New().String()
		req := httptest.NewRequest(http.MethodPut, "/tags/"+tagID, strings.NewReader(`{"name":"Updated"}`))
		req.Header.Set("Content-Type", "application/json")
		req = withChiParams(req, map[string]string{"id": tagID})
		req = withSiteID(req)
		rest.AdaptHandler(h.Update).ServeHTTP(rec, req)

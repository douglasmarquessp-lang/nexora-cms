...
### Sprint 6.8 — SEO Gate desbloqueado estruturalmente (AIWorkSimple, 2026-08-11)

**Objetivo:** corrigir estruturalmente o bloqueio do SEO gate do AIWorkSimple (score ~58-72 < mínimo 80) SEM reduzir `SEO_MIN_PUBLISH_SCORE`, sem desativar o gate e sem bypass — artigos bons passam >= 80 naturalmente (validado: 89.05; sem imagem cai para 85; sem keyword cai mais).

**Causas-raiz encontradas no Sprint 6.7 (produção):**
1. Keyword sempre `deriveKeyword(title)` — pegava a palavra mais longa do título ("simple"/"hours"), nunca a do corpo; gate analisava frase rara → densidade 0.19-0.67% → keyword_score 60-80.
2. `PublishGateInput` não transportava `AuthorName` → EEAT perdia 20 pts de Expertise sem autor (por 6.6 o `SEO_DEFAULT_AUTHOR` nem existia).
3. Imagem só em `featured_image_url` (metadata), nunca no corpo → `analyzeImages` (5%) ficava 30/100 — exceto no caminho em que o enhancer busca no Pexels (o <figure> ia pro corpo, mas não havia teste provando).
4. Sem log estruturado de breakdown por dimensão — diagnóstico impossível em prod. (logs agora INFO `publish gate: seo score breakdown` com title/meta/headings/keyword/readability/internal/external/eeat/images/overall + issues.)

**Mudanças de produção:**

- `internal/modules/seoengine/analyzer.go` — `FocusKeyword(title, body)` NOVO (substitui a derivação burra no gate inline): rank por (1) frases de 2 termos do título presentes no corpo — stopwords internas permitidas ("Marketing de Conteúdo" → "marketing de conteúdo", NUNCA "pequenas empresas" por causa do "de" quebrar a adjacência) — ranked por contagem de ocorrências → length → posição no título; spans limitados a 2 termos de conteúdo (frases curtas, query-like; trailing stopword nunca emitida); (2) termo único mais repetido no corpo; (3) `deriveKeyword` como último recurso. Nunca inventa — o keyword existe no título e (quase sempre) repetido no corpo. Evita os dois traps: generic-longest-word ("simple"/"hours") e phrase-rare-1-hit ("every week"/"small teams" perdem para "ai tools" com 6+ ocorrências).
- `internal/modules/seoengine/analyzer.go` — stopWords += "every", "each" (eram os buracos que deixavam "every week"/"hours every" vencerem).
- `internal/modules/seoengine/service.go` — `CheckPublishScore`/`CheckPublishScoreWithIssues`: `Keyword`/`AuthorName` do input + fallback `s.defaultAuthor` (config) + `FocusKeyword` quando keyword vazio + `logScoreBreakdown` (dimensões + issues; deterministic).
- `internal/modules/seoengine/enhance.go` — `embedPlainImage` (NOVO): imagem FORNECIDA PELO CALLER (FeaturedImageURL da media library/pipeline) agora também é embutida no corpo como `<figure><img alt=deriveImageAlt>`, não só `featured_image_url` — sem isso analyzeImages nunca a via na hora do gate. `featuredAlt` vazio → `deriveImageAlt(title)` (honesto, ~10 palavras, sem keyword-stuffing).
- `internal/ai/pipeline.go` — `runDraft`: instruções de estilo injetadas no prompt (EN: sentences <20 words, paragraphs 2-4 sentences, active voice, exactly one H1, ≥2 H2, ≥1 H3; PT equivalente) — nunca manipulam o cálculo Flesch, apenas orientam o modelo a produzir texto legível.
- `seoengine/eeat.go`, `service.go`, config: já tinham `AnalyzeEEAT` + sinais; agora o gate inline alimenta `AuthorName` (input > defaultAuthor "seoengine" do config → `SEO_DEFAULT_AUTHOR`, env, nunca hardcoded). Wire completo em `cmd/api/main.go` (publisherSvc recebe gate via seoengine; PublishGeneratedRequest repassa `Keyword` + `AuthorName` do enhancer: `ContentEnhancement.Keyword`/`AuthorName` novos).
- `internal/modules/publisher/{enhance.go,gate.go,service.go}` — `ContentEnhancement{Keyword, AuthorName}`, `PublishGateInput{Keyword, AuthorName}` (9º/10º args de `checkPublishGate`); `PublishGeneratedArticle` repassa `enh.Keyword` e `enh.AuthorName` (com fallback `s.defaultAuthor` do publisher).

**Fix de gap encontrado nos testes:** `TestGate keyword emissions` — "5 Simple AI Tools..." com corpo real: "every week" (1 hit) vencia "ai tools" (9 hits) por length-first. Heurística trocada para count-first (ver FocusKeyword acima). `Test focusKeyword` PT: "economizar horas" vs "horas no trabalho" — tie count 1 → length 17 = 17 → earlier position ganha.
